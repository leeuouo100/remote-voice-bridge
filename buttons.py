...
def resolve_button(
    ...
    # 原样直通：程序读到这个键但**什么都不发**。
    #
    # ⚠ v1.0.11 改写了这条的语义。以前写的是"遥控器自己的按键 Windows 本来就
    #   收得到，再注入一次只会变成双份" —— 那个前提是**错的**：遥控器的按键发在
    #   厂商自定义页上，Windows 根本不处理厂商页，原生按键并不会自己生效。
    #   （详见 config.NATIVE_TARGET_HINT 与 README 的 v1.0.11 段。）
    #   所以「原样直通」对遥控器按键实际等于"什么都不做"，跟禁用看着一样。
    #   默认映射也因此不再用它（方向键改成真正注入方向键）。
    if mapped == "native":
        return False
