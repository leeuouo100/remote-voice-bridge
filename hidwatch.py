...
class ReportWatcher:
    ...
    def summary_lines(self) -> list[str]:
        ...
        if real:
            ...
            # ⚠ 2026-09-23 改写。旧文案是「或按键报告走在**厂商页而该路被系统占用**」——
            #   两个毛病：① 厂商页**没人占**，我们自己就打得开，拿它当借口是错的；
            #   ② 把「这一路根本没观测到」和「观测了、确实 0 条」混成一句话 ——
            #   这正是本项目反复踩的「读不到当成没有」。所以现在**先拆开说**。
            opened = [c for c in self.collections if c.opened]
            unseen = [c for c in self.collections if not c.opened]
            A("  → ⚠ 能观测到的通道一条都没收到。先把两件事拆开：")
            if opened:
                A("     · **确实观测了**的几路："
                  + "、".join(c.key.strip() for c in opened))
                A("       这几路是**真 0** —— 遥控器没往这些页发报告。")
            else:
                A("     · ⚠ **一路都没观测到** —— 这轮等于没测，不能下任何结论。")
            if unseen:
                A("     · **没打开、等于没观测**的几路：")
                for c in unseen:
                    A(f"        - {c.key.strip()}："
                      f"{c.status or c.open_error or '原因未记录'}")
                A("       键盘页 / 鼠标页打不开是**正常**的（Windows 独占了它们）——")
                A("       但影响不同：键盘那一层由上面的**键盘钩子**覆盖（不是空白），")
                A("       鼠标那一层才是真的没测。")
            A("     ⇒ 到这里能下的结论只有：「**在能观测到的页上，遥控器没有发报告**」。")
            A("       剩下的可能只剩两条，而**这两条从用户态分不开**：")
            A("        ① 报告落在鼠标页（我们看不见）；")
            A("        ② 报告本该走 0x1812 的 Report 特征，但**没有任何人订阅它**")
            A("           （HOGP 占着服务却没订阅）⇒ 设备干脆不发。")
            A("     请把**整份**报告发出来（别只发结论行）——上面的分法要看到")
            A("     「哪几路打开、哪几路没打开」才能判。")
        if self.hook_error:
