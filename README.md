...
**按键映射现在真的能用了 —— 方向键、返回、Home、音量、YouTube / Netflix / 电源 / 信源，全都能映射。**

之前「除了语音键，其它按键一个都没反应」的**根因终于找到了**：

遥控器（VID `18D1` / PID `9450`）暴露 5 路 HID 集合 —— 键盘 / 消费类 / 鼠标 /
**厂商页 `0xFF01`** / **厂商页 `0xFF80`**。它把**所有按键**都发在两个厂商自定义页上，
而 **Windows 完全不处理厂商页**（只认键盘页 / 消费类页 / 鼠标页）。
于是这些键既不产生键盘事件、也不产生媒体键事件 —— **键盘钩子永远看不到它们**，
映射表改到天亮也没用，因为事件根本没进 Windows。
...
remote_hid.py   读遥控器的 HID 厂商自定义页并解码按键（v1.0.11 起按键映射全靠它）
