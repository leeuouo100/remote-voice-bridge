...
> **绝大多数"连不上"其实是配对记录作废了**：配对记录是**按本地蓝牙无线电**存的。
> 换过 USB 口、换过蓝牙模块、同一块 dongle 换了插口，本地地址就变了，旧记录随之失效 ——
> Windows 设置里仍然显示「已配对」，但连不上。
> 处置：设置 → 蓝牙和其他设备 → 删除该设备 → 遥控器长按 **Home+返回 约 3 秒** 进配对 → 重新添加。
...
**配对记录是"按本地蓝牙无线电"存的。** 换过 USB 口、换过蓝牙模块、同一块 dongle
换了插口 —— 本地地址就变了，旧记录随之作废：Windows 设置里照样显示「已配对」，
但蓝牙栈造不出可用设备，怎么重试都连不上。（2026-09-15 真机就是这个。）

v1.0.9 起日志里会直接写出两个地址和处置，先看：

```bat
%APPDATA%\remote-voice-bridge\bridge.log
```

处置：设置 → 蓝牙和其他设备 → **删除「Chromecast Remote」** →
