...
### v1.0.14（当前）—— 修回 v1.0.13 把语音输入改坏的问题

**装 v1.0.13 的人：语音输入完全用不了，请务必升到这一版。**
现象是「按遥控器语音键什么都不发生」——不报错、托盘正常、控制台波形还在跳，
但输入法一次都没被叫起来。**没有任何设置能绕开**，只能升级。

根因是**两行代码的顺序**：程序在收到 `audio_start` 时会补发一次 `MIC_OPEN`
（遥控器只认这个命令才推音频），而补发成功时会顺手记下"刚发过开麦命令"的时刻；
v1.0.13 把这段补发放**在**"判断这一下是不是回声"**之前** —— 于是**你那次真按键
自己把自己判成了回声**、当场被吞掉。而每次松手都会让状态复位，下一次按键又重来一遍，
所以**连按 10 次也一次都启动不了**。

> 顺带纠正一个流传的版本结论：这个顺序 **v1.0.10 / v1.0.11 / v1.0.12 三版都有**，
> 它们"能用"其实是**靠回声顶上**（真按键被吞 → 旧版吞掉时会把时刻清零 →
> 紧接着的回声就把会话启动起来）。清零点正是旧版能跑起来的原因，
> 所以时序一歪就会出现那 4/10 次的「按一下、刚开口就被关掉」。

**这一版怎么修的**：

| 改动 | 从 | 到 |
|---|---|---|
| 顺序 | 先补开麦、后判回声 | **先判回声、后补开麦**（补开麦只在"真按键开始新一段"里） |
| 回声窗口宽度 | 1.5 秒 | **0.8 秒**（实测回声 ≤350ms、真按键 ≥1s，两边都留余量） |
| 窗口怎么过期 | 吞掉时清零 / 被刷新 | **只靠时间过期**，任何事件都不许改它 |

修好之后：热键由**真按键**触发，回声彻底不参与状态机（不再是"靠回声顶替按键"）。

闸门 `tools/check_voice_session.py` 新增 3 项断言 + 1 条反例，专门钉这个顺序 ——
**这类"两行都合法、换行位置错了就静默失效"的 bug，只能靠闸门拦，人记不住。**

详见 [CHANGELOG.md](CHANGELOG.md) 的 v1.0.14 一节（含真机日志对照：成功 vs 失败）。
...
**这一版把"必须摸鼠标"这件事去掉了，但没有替你做"要不要发"这个决定：**

| 现在的默认行为 | 说明 |
|---|---|
| 按语音键结束 → 文字落进输入框 → **停在那儿** | 你想改就改，想发的时候**按键盘回车**（手本来就在键盘上，不用去够鼠标） |
| **不自动发送** | ⚠ **默认关**，这是有意的 |

🔴 **为什么默认不自动发**（2026-09-17 武哥否掉了原来"默认开"的设计，他是对的）：
自动发送会把**还没想好的话**替你发出去。真实场景：说到一半去倒杯水，
回来想接着改、改完再发 —— 程序早把半截话发进真人对话里了。
**这个代价远大于"要自己动手发一下"。宁可不发，也不能替你说出你还没决定要说的话。**

真想"说完即发"的人，可以在控制台「设置」页把开关打开（**开关在，只是默认不替你做主**）。
打开之后仍有四道保护：
...
| 挂着忘了关（超时收尾） | **永不发送**（那 10 分钟很可能录到环境音/旁人的话） |
| 配置里读不到这个开关 | **按"不发"处理**（兜底方向必须是安全的那一边） |

> 「给遥控器上某个键绑一个回车」仍然走不通（除语音键以外的按键在 Windows 上收不到报告，
> 见下面 v1.0.12 的推翻结论）。**哪天那条路通了，把它绑成一个「发送」键才是最理想的** ——
> 既不用摸鼠标，又完全由你决定什么时候发。现在先用键盘回车顶着。
> v1.0.13 之前未发布过，所以不存在"老用户被默认值改到"的问题；
> 配置版本仍升到 3，`_migrate` 会把任何被打开过的自动发送**归位成关**。

> 闸 `tools/check_send_after_voice.py`（**27 项静态 + 9 项行为级 + 14 条反例自证**）。
> ⚠ 其中**第一条行为级用例就是「默认配置下到点了也绝不发」** —— 默认值一旦回退成开，先红的就是它。
> 行为级那一层会把 `main` 真 import 进来、换假时钟和假按键注入（**绝不真注入**），
> **真调一遍**看它到点有没有真的发 —— 静态检查证明不了"它真的会发"，也证明不了"它真的不发"。
...
### 🔴 按键这件事已经查到底了：五个集合**一条报告都没来**

2026-09-22 / 23 两轮真机取证：

| 证据 | 结果 |
|---|---|
| 115302 行日志里的 `🔘 HID 按键` | 343 条，**全是字母/数字/修饰键**（物理键盘 + 程序自己注入的热键回环） |
| 遥控器独有的键 `up` `down` `home` `volume up/down/mute` | **一次都没出现** |
| `📊 HID 通道审计`（已打开的 3 路集合） | 13 条，**全部「累计 0 条原始报告」** |
| ATVV 控制通道的未处理指令 | **0 条**（按键也没走这条路） |

⇒ 遥控器**声明了**键盘集合（所以 Windows 装了 `kbdhid`），但按键从来没有到过 Windows。
注意这里有个容易被人反驳的缝，先说清：

> 「我们能读的那 3 路都是 0 条」**不等于**「遥控器什么都没发」—— 唯一读不到的是
> 键盘集合那一路（被 HOGP 独占）。
> 但**那一路若真有报告，Windows 会把它变成键盘事件**，而 `main.py` 的全局钩子
> 会把这些事件记成 `🔘 HID 按键`（343 条，去重后覆盖 8 天全部会话）。
> 那 343 条里**一条遥控器独有的键都没有**（`up`/`down`/`home`/`volume*` 零出现），
> 全是物理键盘和程序自己注入的键。
> ⇒ 所以"没发"这个结论不是推断，是**从 Windows 侧反证出来的**。

而要自己读原始报告，就得独占 `0x1812` —— 它被 Windows 的 HOGP 驱动占着，两条提权路都试过：

- `Disable-PnpDevice` → `DISABLE_FAIL: 不支持`（`ERROR_NOT_SUPPORTED`）
- `pnputil /disable-device` → `Cannot disable critical system device`（**rc 还是 0**）
- devnode `DevNodeStatus=0x0180000A` **缺 `DN_DISABLEABLE`** ⇒ 设备管理器里「禁用设备」同样是灰的
- ⇒ 用户态没有任何办法让 Windows 放手 ⇒ 自己订 `0x1812` 永远 `ACCESS_DENIED`

**另外两个流传过、但已被否掉的修法**：

- ❌ 「v1.0.11 起程序自己读那两个厂商页就能解」——真机实测：页面打得开、**报告 0 条**。
- ❌ 「换个 API 重新 bonding，绕开 Windows 的 HID 配对流程」——配对仪式和服务归属是两件事：
  `0x1812` 被占是因为**设备被判定成 HID 设备**（`hidbthle` / `mshidumdf` 绑定），
  跟谁执行配对无关。而且 `bleak` 的 `pair()` 遇到已配对的设备**直接 return**，
  对这个遥控器是 no-op（详见 [CHANGELOG](CHANGELOG.md)）。

⚠ **一句话结论：「纯软件」和「用这个遥控器的按键」目前不可兼得。**
想要按键就只能加内核过滤驱动或换一个标准 HID 设备（两条都已被否决）。
遥控器上现在只剩**语音键**好用 —— 它走 ATVV，不走 HID。

### 想自己复验这个结论（60 秒，不改任何东西）
