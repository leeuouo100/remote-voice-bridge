...
| `check_send_after_voice.py` | 语音说完之后**能自己发出去**：默认开、发送延迟 ≥300ms（太小会在文字落进输入框**之前**就回车，等于把刚说的话弄丢一次）、开始新一段要取消待发送、零音频帧不发送（误触保护）、三个字段进了 `_get_cfg` 白名单。含 **5 条反例自证** | v1.0.13 武哥原话：「说完还要去电脑上按鼠标点发送，已经完全没有了 voice coding 的感觉」。遥控器除语音键以外的键在 Windows 上收不到报告，所以**不靠按键** —— 靠"本程序本来就知道这段说完了"这一点 |
