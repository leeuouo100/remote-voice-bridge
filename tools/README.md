...
| 脚本 | 回答什么问题 | 怎么用 |
|---|---|---|
| `probe_hogp_state.py` | 「设备管理器一切正常、配对也显示已配对，为什么按键一个都不到」—— **按键为什么没有接收方**。四段：① **配对账本有没有 BLE 密钥材料**（没有 `LTK` ⇒ 链路加不了密 ⇒ HOGP 订阅不到 HID 报告，遥控器发出去也没人接）② Windows 里同一个遥控器有几条已配对条目 ③ 该怎么做（**含把遥控器按进配对模式的正确手法**）④ 各 devnode 的**在场**判定（cfgmgr32，能分出"现在活着的那一代"和注册表里的幽灵） | `python tools\probe_hogp_state.py`（**只读，不用按键、不用退桥程序**）/ `--selftest`（只验判词，不碰注册表）/ `--rebind`（需管理员，非破坏性重枚举） |
